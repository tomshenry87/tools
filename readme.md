...
| | `--firmware` | None | Only show displays not on this firmware version in the table |
...
The `--firmware` flag filters the terminal table to only show displays whose firmware version **does not match** the provided string. This makes it easy to immediately identify which units in a fleet are behind without manually scanning through the full results.

The JSON output file always contains the complete results for all displays regardless of the filter.

```bash
# Show only displays not running firmware 6.0.81.60
python3 bravia_firmware.py --firmware 6.0.81.60

# Combined with a custom CSV and output file
python3 bravia_firmware.py -i displays.csv --firmware 6.0.81.60 -o results.json

# With PSK authentication
python3 bravia_firmware.py -i displays.csv -k 0000 --firmware 6.0.81.60
```

**Behaviour details:**

- The version string is matched against the parsed `firmware_version` field (e.g. `6.0.81.60`), not the raw `fwVersion` build string
- Displays that failed to respond (firmware `N/A`) are included in the filtered output, since their firmware version could not be confirmed
- The header block prints a `Filter:` line confirming the active version string
- The table title is appended with `— Mismatched: N/total` for a quick count
- The summary footer prints a `Firmware filter: ... | Showing: N of N displays` line above the standard counts

**Example terminal output with filter active:**
...
```
  ...
  Auth:    None (fallback: 1234)
  API:     1.7 -> 1.4 -> 1.0 (automatic fallback)
  Filter:  Showing displays not on firmware 6.0.81.60

  =====================================================================
         Sony Bravia BZ40H/BZ40L — Firmware Query Results — Mismatched: 3/20
  =====================================================================
  ...

  Firmware filter: 6.0.81.60  |  Showing: 3 of 20 displays
