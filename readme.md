...
### `rlink_firmware.py` — Middle Atlantic RackLink PDUs
Queries Middle Atlantic RackLink Select Series PDUs (RLNK-915R, RLNK-415R, RLNK-215) via their HTTP/HTTPS settings JSON endpoint. Retrieves firmware version, model, serial number, MAC address, device name, and outlet names/states. Supports a `--firmware` filter flag to show only devices that don't match a target version, making it easy to identify units needing an update.
