...
def debug_print(host, label, data):
    if not DEBUG:
        return
    dir_col   = MGT if "SEND" in label.upper() else GRN
    hex_width = 16
    header    = f"DEBUG [{host}] {label} ({len(data)} bytes)"
    box_w     = max(len(header) + 4, 62)
    print(f"\n{YLW}  +-- {dir_col}{header}{YLW} {'-' * max(0, box_w - len(header) - 4)}+{RST}")
    if not data:
        print(f"{YLW}  |{RST}  (empty)")
    else:
        for offset in range(0, len(data), hex_width):
            ...
            print(f"{YLW}  |{RST} {DIM}{offset:04X}{RST}  {CYN}{hex_cols}{RST}  {GRN}|{ascii_col.ljust(hex_width)}|{RST}")
        decoded = data.decode("ascii", errors="replace")
        visible = (decoded
                   .replace("\r", f"{RED}{{CR}}{RST}")
                   .replace("\n", f"{RED}{{LF}}{RST}")
                   .replace("\x00", f"{RED}{{NUL}}{RST}"))
        print(f"{YLW}  |{RST}  decoded : {visible}")
    print(f"{YLW}  +{'-' * box_w}+{RST}\n")
            ...
            for key, (command, pattern) in targets.items():
                ...
                for line in raw.splitlines():
                    ...
                    if m:
                        ...
                        break
                print(f"  {GRN}parsed [{key}]{RST} => {parsed!r}\n")

    except (ConnectionRefusedError, socket.timeout, OSError) as exc:
        print(f"\n  {RED}Connection failed: {exc}{RST}\n")
    sys.exit(0)
...
def main():
    ...
    label = "  [DEBUG ON]" if DEBUG else ""
    print(f"\nQuerying {len(hosts)} device(s) via Kramer Protocol 3000 (TCP){label}...\n")
